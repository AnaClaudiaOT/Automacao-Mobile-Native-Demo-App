...
describe("Navegação entre telas", () => {
  ...
  it("Deve navegar para a página Home e validar a mensagem", async () => {
  ...
  it("Deve navegar para a página Login e validar a mensagem", async () => {
  ...
  it("Deve navegar para a página Webview e validar a mensagem", async () => {
    await NavigatePage.navigateToWebview()
    await browser.pause(5000) // Aguarda a navegação
    const webviewMessage = await NavigatePage.getWebviewMessageText()
    expect(webviewMessage).toBe(
