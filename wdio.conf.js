...
exports.config = {
  ...
  afterTest: async function (test, context, { error }) {
    ...
    try {
      ...
      addAttachment(
        "Evidência - " + test.title,
        fs.readFileSync(screenshotPath),
        "image/png"
      )
    } catch (err) {
      console.error(
        `Erro ao salvar ou anexar a screenshot para o teste "${test.title}":`,
        err
      )
    }
  ...
  onPrepare: function (config, capabilities) {
    ...
    const automationName =
      capabilities[0]["appium:automationName"] || "Não especificado"
    const appPath = capabilities[0]["appium:app"] || "Não especificado"
    const chromedriverExecutable =
      capabilities[0]["appium:chromedriverExecutable"] || "Não especificado"
    ...
    try {
      ...
      console.log(
        `Arquivo environment.properties gerado em ${environmentFilePath}`
      )
    } catch (error) {
